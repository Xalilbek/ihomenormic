...
class KnowledgebaseitemsController extends \Phalcon\Mvc\Controller
{
		...
		if ($data)
		{
			foreach($data as $value)
			{
				$records["data"][] = array(
					...
					'<a href="'._PANEL_ROOT_.'/knowledgebaseitems/edit/'.$value->id.'" class=""><i class="la la-edit"></i></a>'.
		...
		if((int)$this->request->get("save") == 1)
		{
			...
			else
			{
				$userInsert = [
					...
					"active"					=> 1,
					"is_deleted"				=> 0,
					"created_at"				=> KnowledgebaseItems::getDate()
				];

				$id = KnowledgebaseItems::insert($userInsert);
	...
	public function editAction($id)
	{
				...
				if($tempFiles)
				{
					foreach($tempFiles as $value)
					{
						$document = [
							...
							"creator_id"		=> $this->auth->getData()->id,
							"moderator_id"      => (int)$value->moderator_id,
							"knowledgebase_id"  => (string)$data->_id,
							"uuid"              => $value->uuid,
							"type"              => $value->type,
							"filename"          => $value->filename,
							"is_deleted"        => 0,
		...
		$this->view->setVar("documents", $documentsQuery);
		$this->view->setVar("puid", $puid);
	}
