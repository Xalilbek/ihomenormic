...
class DocumentsController extends \Phalcon\Mvc\Controller
{
    public function uploadAction()
    {
        ...
        else if (!in_array(strtolower($filetype), array("jpeg", "jpg", "png", "gif", "pdf", "doc","docx", "xls", "xlsx", "txt")))
        {
            $error = $this->lang->get("FileTypeNotAllowed", "This file type is not allowed");
        }
        else if ($photo['size'] > 50*1024*1024)
        {
            $error = $this->lang->get("FileSizeMaxLimit", "File size cann`t be larger than 30 MB");
        }
        else
        {
            if(strlen($this->request->get("uuid")) > 0){
                $uuid   = trim($this->request->get("uuid"));
            }else{
            ...
            {
                $fullFile = $dir2.$filename.".".$filetype;
                move_uploaded_file($photo["tmp_name"], $fullFile);
    ...
    public function rotateAction()
    {
        ...
        $photo = Documents::findById($pid);

        if($photo)
        {
            $path = FILE_DIR.@$photo->uuid.'/'.(string)@$photo->_id.'/';
            $orginal = $path."org.jpg";
            chmod($orginal, 0777);
            list($width, $height) = getimagesize($orginal);
            $proportion = $width / $height;

            //$angle = ($_POST['angle'] == '1') ? 90: 270;
            $angle  = 90;
            $im     = new Image();
            $im->load($orginal);
            $im->rotate($angle);
            ...
            Image::resize($orginal, 300, 300, $path.'medium.jpg', 90);
            Image::resize($orginal, 100, 100, $path.'small.jpg', 90);

            $data = [
                "status"	=> "success",
                "path"      => $path,
                //'url'		=> '/photos/'.$uid.'/'.$photo_id.'/thumb.jpg'
            ];
        }
        else
        {
    ...
    {
        $photo = Documents::findFirst([["_id" => Documents::objectId($this->request->get("id"))]]);


        if(!$photo)exit("no photo: ".$this->request->get("id"));
