...
class ContractsController extends \Phalcon\Mvc\Controller
{
	...
	public function listAction()
	{
		...
		if ($data)
		{
			foreach($data as $value)
			{
				...
				$list[] = '<a href="'._PANEL_ROOT_.'/contracts/send/'.$value->id.'" class=""><i class="la la-envelope"></i></a>';
	...
	public function sendAction($id)
	{
		...
		{
			$layout = str_replace("{citizen_name}", $citizenData->firstname." ".$citizenData->lastname, $layout);
			$layout = str_replace("{citizen_number}", $citizenData->phone, $layout);
			$layout = str_replace("{citizen_address}", $citizenData->address, $layout);
		}
		if($employeeData = Users::getById($employee))
		{
			$layout = str_replace("{employee_name}", $employeeData->firstname." ".$employeeData->lastname, $layout);
			$layout = str_replace("{employee_number}", $employeeData->phone, $layout);
			$layout = str_replace("{employee_address}", $employeeData->address, $layout);
		}
