var PPermission = {
    ...
    {
        var h = '';
        console.log(PPermission.permissions);
        for (var key in PPermission.permissions){
            var permission = PPermission.permissions[key];
            h += '<div class="row '+((permission.has_margin) ? "permission-margin-top": "")+'">';
            h += '<div class="permission-title">'+permission.title+'</div>';

            for (var type in permission.permissions){
                var typeValue = permission.permissions[type];
                h += '<div class="permission-type">';
