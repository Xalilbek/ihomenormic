...
class ActivitiesController extends \Phalcon\Mvc\Controller
{
	public function listAction()
	{
		...
		if ($data)
		{
			...
			foreach($documentsQuery as $value)
				$documents[(string)$value->activity_id][] = [
					"id"	=> (string)$value->_id,
					"type"	=> "image",
					"name"	=> (string)htmlspecialchars($value->filename).'.'.$value->type,
					"url"	=> FILE_URL."/upload/".(string)$value->uuid."/".(string)$value->_id."/medium.jpg",
				];
			...
			foreach($data as $value)
			{
				...
				$records[$month][] = [
					...
					"files"		=> count($documents[(string)$value->_id]) > 0 ? $documents[(string)$value->_id]: false,
				];
	...
	public function addAction()
	{
		...
		if(strtolower($this->request->get("timezone")) == "da"){
			$unixtime	= $this->lib->dateFomDanish($date)." 00:00:00";
		}else{
